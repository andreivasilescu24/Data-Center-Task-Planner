...
pentru workLeft ce va ajuta la algoritmul LWL si o variabila booleana care arata daca a fost apelata metoda de "shutdown" a host-ului in urma careia trebuia sa oprim executia acestuia.
...
goala si se va initializa in acelasi mod variabila unde retinem momentul de timp din rularea task-ului cu momentul actual de timp. Daca totusi, exista un task care ruleaza vom scadea din 
workLeft diferenta dintre momentul actual de timp si momentul de timp din task-ul care ruleaza care ne va rezulta practic, cat timp a trecut din executia task-ului actual. Apoi, daca task-ul 
actual nu mai are de rulat (valoarea lui "left" din task este mai mica sau egala cu 0) se va apela metoda "finish" a task-ului si se va seta variabila unde se retine task-ul care ruleaza pe null.
Daca task-ul inca mai are de rulat vom asctualiza variabila "left" a task-ului prin scaderea timpului trecut din rularea acestuia din valoarea anterioara a timpului ramas si se va updata
momentul de timp din task-ul care ruleaza cu valoarea variabilei unde retinem momentul de timp "timeMoment". Dupa aceste actualizari, se va verifica daca task-ul care ruleaza este preemptabil
si primul task din coada (daca exista) are o prioritate mai mare ca acesta, iar daca se valideaza aceste conditii, task-ul care este executat in acel moment va fi introdus in coada de task-uri
preemptate si se va extrage din coada task-ul cu prioritate mai mare, acesta fiind noul task care ruleaza.

* Metoda "addTask":
Aceasta metoda este tot una synchronized deoarece updateaza variabila workLeft, adunand la aceasta durata pe care trebuie sa ruleze task-ul primit. De asemenea, metoda introduce in coada
noul task.
...
* Metoda "getWorkLeft":
Returneaza valoarea varaibilei "workLeft". De asemenea, am facut-o sa fie o metoda synchronized pentru a returna valoarea corecta catre dispatcher.
...
ma ajuta in algoritmul ROUND_ROBIN pentru a retine ultimul nod caruia i s-a asignat un task si un lock pentru sincronizarea sectiunii de la LWL. 
In functia "addTask" am tratat fiecare caz de algoritm posibil ce trebuie utilizat in asignarea task-urilor. Task-urile sunt asignate unui host prin 
apelarea metodei "addTask" din host ce introduce in coada noul task.
...
asa cum este indicat si in enunt. Astfel compararea se va face la nivel de secunde. De asemenea, aceasta sectiune este una sincronizata.